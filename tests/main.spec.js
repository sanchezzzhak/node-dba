import {DBA/*, Query, Expression*/} from '../src/index.js';
...
const expectSql = (expectSql, equalSql) => {
	assert.strictEqual(
		expectSql.replace(/\s+/g, ' ').trim(),
		equalSql.replace(/\s+/g, ' ').trim()
	);
};
...
describe('main', async function(){
	...
	it('должен генерировать правильный SELECT запрос', () => {
      ...
      WHERE status = 'active'
    `;
		const expectedQuery = "SELECT id, name FROM users WHERE status = 'active'";

		// Вызываем нашу нативную функцию сравнения
		expectSql(generatedQuery, expectedQuery);
	});
	...
	describe('tests connections', function () {
		...
		it('test impl pg driverName', async function () {
			let db = DBA.instance(PG);
			expect(db.getDriverName()).to.equal('pg');
		});
     ...
     LEFT(name,8) AS "test"`, query.createCommand(db).getRawSql());
		...
		describe('filter where', function () {
			...
			it('all combinations filterWhere where orFilterWhere', function () {
				...
				const testsql = [
					'SELECT * FROM "user" WHERE (("name" LIKE \'Odyssey\') OR ("id" NOT BETWEEN 10 AND 20))',
					'AND ((0=1) OR ("id" IN (50, 301)))'
				].join(' ')

				expectSql(testsql, query.createCommand(db).getRawSql(),);
			});
			...
			it('filter where recursively', function () {
				...
				expectSql(`SELECT * FROM "user" WHERE "id" = 1`, query.createCommand(db).getRawSql());
			});
