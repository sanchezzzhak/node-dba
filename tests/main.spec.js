...
describe('tests connections', function () {
  ...
  it('test from alias object for Query', function () {
	...
	expectSql(`SELECT * FROM "users" "u", "payments" "p"`, query.createCommand(db).getRawSql());
  });

  it ('test from for Map', function() {
	let tables = new Map();
	tables.set('u', 'user');
	tables.set('p', 'post');
	...
	expectSql(`SELECT * FROM "user" "u", "post" "p"`, query.createCommand(db).getRawSql());
  });
